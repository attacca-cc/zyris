...
| 신뢰 경계 | **TOFU 키 고정.** attacca를 소개 신뢰 앵커로 쓰되 첫 연결에서 상대 키를 고정하고, 이후 바뀌면 거부한다 |
...
```
A가 B에게 처음 보낸다   → attacca가 준 B의 EndpointId를 그대로 쓴다 → 성공하면 고정
A가 B에게 다시 보낸다   → 고정해 둔 것과 다르면 → err peer_key_changed, 전송 중단
```

- 자리: `~/.local/share/zyris/peers.json` (0600). `{ node_id, endpoint_id, first_seen, last_seen }`.
- **양쪽 다 고정한다.** B도 들어온 A의 EndpointId를 고정한다.
- 바뀌었을 때 자동으로 푸는 길은 두지 않는다. 사람이 파일에서 지워야 한다 — 조용히 받아들이면
  고정하는 의미가 없다.

**이 설계가 막는 것**: 네트워크 도청, 릴레이의 내용 열람, 수동 로깅, 전송 중 변조.
**남는 것**: 최초 1회의 능동적 MITM. 그때도 두 번째 전송에서 키 불일치로 드러난다.
