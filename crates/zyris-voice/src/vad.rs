...
mod tests {
    ...
    fn recorded(name: &str) -> Vec<f32> {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/audio").join(name);
        ...
        // The whole crate is written for one format. A fixture in another would be read as
        // nonsense rather than refused, which is the failure this assertion exists to prevent.
        assert_eq!(
            (rate, channels, bits),
            (SAMPLE_RATE, 1, 16),
